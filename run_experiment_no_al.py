...
import torch_utils
from dataset_enum import DatasetEnum, get_targets
...
def main():
    ...
    if not reduced_train_length:
        reduced_train_length = len(dataset.available_dataset)

    print(f"Training with reduced dataset of {reduced_train_length} data points")
    if not args.balanced_training_set:
        dataset.active_learning_data.acquire(
            dataset.active_learning_data.get_random_available_indices(reduced_train_length)
        )
    else:
            ...
            list(
                itertools.chain.from_iterable(
                    torch_utils.get_balanced_sample_indices(
                        get_targets(dataset.available_dataset), dataset.num_classes, num_samples_per_class
                    ).values()
                )
            )
        )

    if len(dataset.train_dataset) < args.epoch_samples:
        sampler = RandomFixedLengthSampler(dataset.train_dataset, args.epoch_samples)
    else:
        sampler = data.RandomSampler(dataset.train_dataset)

    test_loader = torch.utils.data.DataLoader(
        dataset.test_dataset, batch_size=args.test_batch_size, shuffle=False, **kwargs
    )
    train_loader = torch.utils.data.DataLoader(
        dataset.train_dataset, sampler=sampler, batch_size=args.batch_size, **kwargs
    )

    validation_loader = torch.utils.data.DataLoader(
        dataset.validation_dataset, batch_size=args.test_batch_size, shuffle=False, **kwargs
    )
