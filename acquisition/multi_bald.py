...
import computation.joint_entropy.exact as joint_entropy_exact
import computation.joint_entropy.sampling as joint_entropy_sampling
import math
import time
...
def compute_acs_fw_batch(
    ...
    B, K, C = list(logits_B_K_C.shape) # (pool size, mc dropout samples, classes)
    py = logits_B_K_C.exp_().mean(dim=1)

    num_projections=10
    gamma=0.7
    assert K >= num_projections
    cs = ProjectedFrankWolfe(py, logits_B_K_C[:, :num_projections, :], num_projections, gamma=gamma)
...
def compute_fass_batch(
    ...
    B, K, C = list(logits_B_K_C.shape)
    probs_B_C = logits_B_K_C.exp_().mean(dim=1)
...
def compute_ical(
    ...
    dist_B_K_C = tdist.categorical.Categorical(logits_B_K_C.view(-1, C)) # shape B*K x C
    ...
    score_sort = torch.sort(scores_B, descending=True)
    score_sort_idx = score_sort[1]
    score_sort = score_sort[0]
    ...
    for ackb_i in range(max_greedy_iterations):
        ...
        global_acquisition_bag += [i.item() for i in subset_split.get_dataset_indices(winner_idxes)]
        acquisition_bag_scores += [s.item() for s in hsic_scores[winner_idxes]]
        print('winner score', scores_B[winner_idxes].mean().item(), ', hsic_score', hsic_scores[winner_idxes].mean().item(), ', ackb_i', ackb_i)
        if batch_kernel is None:
        ...
        scores_B[winner_idxes] = -math.inf
        score_sort = torch.sort(scores_B, descending=True)
        score_sort_idx = score_sort[1]
        score_sort = score_sort[0]
        if hsic_resample:
