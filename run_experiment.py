...
from acquisition.methods import AcquisitionMethod
...
from torch_utils import get_base_indices, get_targets
import torch.utils.data as data
import numpy as np
import random
from acquisition.functions import AcquisitionFunction, compute_pair_mi, generate_sample, compute_mi_sample
from sampler_model import reduced_eval_consistent_bayesian_model
...
def main():
    ...
    dataset.get_experiment_data(args.initial_samples, reduced_dataset, samples_per_class, validation_set_size,
                                balanced_test_set, balanced_validation_set)
    dataset.prepare_data_loaders(args.epoch_samples, args.batch_size, args.test_batch_size, args.scoring_batch_size, **kwargs)

    store["iterations"] = []
    # store wraps the empty list in a storable list, so we need to fetch it separately.
    iterations = store["iterations"]

    store["initial_samples"] = dataset.initial_samples
    ...
    for iteration in itertools.count(1):
        def desc(name):
            return lambda engine: "%s: %s (%s samples)" % (name, iteration, len(dataset.train_dataset))
        ...
        target_size = max(args.min_candidates_per_acquired_item * args.available_sample_k, len(dataset.available_loader.dataset) * args.min_remaining_percentage // 100)
        subset_split, subset_dataloader, scores_B, logits_B_K_C = reduced_eval_consistent_bayesian_model(
            ...
            available_loader=dataset.available_loader,
            device=device
        )
        print("entropy score shape:", scores_B.numpy().shape)
        entropy_score = scores_B.numpy().mean()
        to_store = {}
        ...
        original_batch_indices = get_base_indices(dataset.available_dataset, batch.indices)
        print(f"Acquiring indices {original_batch_indices}")
        targets = get_targets(dataset.available_dataset)
        ...
        iterations.append(
            dict(
                ...
                chosen_samples_orignal_score=batch.original_scores,
        ...
        dataset.active_learning_data.acquire(batch.indices)

        num_acquired_samples = len(dataset.active_learning_data.active_dataset) - len(
            dataset.initial_samples
        )
